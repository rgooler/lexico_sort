...
def score_word_by_alphabet(word, precalcalpha, max_bits):
    ...
    total = 0
    for letter in word:
        # Should be able to AND the lower bits, but this triggers errors
        # And since we shift the bits by max_bits with each pass, 2 letter
        # words are ALWAYS going to sort out before three letter words, and
        # collisions are impossible. Plus, python will scale up to arbitrarily
        # large numbers, so this will keep working as long as you have ram.
